...
void setupLP(CEnv env, Prob lp, Data data) {
	...
	for (int k = 0; k < static_cast<int>(data.n); k++) {
	...
	for (int i = 0 ; i < static_cast<int>(data.n); i++) {
		std::vector<int> idx(static_cast<int>(data.n));
		std::vector<double> coef(static_cast<int>(data.n), 1.0);
		for (int j = 0; j < static_cast<int>(data.n); j++)
		{
		      idx[j] = (data.n*(data.n)) + i*(data.n) + j; //corresponds to variable y_ij: (data.n*(data.n-1)) is the offset needed to get the y_ij's
		}        
		char sense = 'E';
	...
	for (int j = 0 ; j < static_cast<int>(data.n); j++) {
		...
		for (int i = 0; i < static_cast<int>(data.n); i++)
		{
		      idx[i] = (data.n*(data.n)) + i*(data.n) + j; //corresponds to variable y_ij: (data.n*(data.n-1)) is the offset needed to get the y_ij's
		}        
		char sense = 'E';
	...
	for (int i = 0 ; i < static_cast<int>(data.n); ++i ) {
		for (int j = 0 ; j < static_cast<int>(data.n); ++j) {
			...
			idx[1] = (data.n*(data.n)) + i*(data.n) + j;   // y var  (data.n*(data.n-1)) is the offset needed to get the y_ij's
